...
def main():
    ...
    # We need to find the original baseline length to avoid duplicating it.
    # Both datasets append the new samples to the original baseline training set.
    # We can load the original data to find its length.
    import sys
    ...
    # Extract only the newly generated samples from the Gemini dataset
    gemini_new_only = gemini_train.select(range(baseline_len, len(gemini_train)))
    
    # Concatenate BT dataset (which already includes the baseline) with the new Gemini samples
    combined_train = concatenate_datasets([bt_train, gemini_new_only])
    
    logger.info(f"Combined train length: {len(combined_train)}")
    
    # Validation and test remain identical
